...
// 네이버 검색 공통 파라미터 (한국어 우선 설정)
export const SearchArgsSchema = z.object({
  query: z.string().describe("검색어 (한국어 키워드 권장)"),
  display: z.number().min(1).max(100).optional().default(10).describe("한 번에 가져올 결과 수 (기본 10, 최대 100)"),
  start: z.number().min(1).optional().default(1).describe("검색 시작 위치 (기본 1)"),
  sort: z
    .enum(["sim", "date"])
    .optional()
    .default("sim")
    .describe("정렬 방식 (sim: 유사도순, date: 최신순)"),
  country: z.string().optional().default("kr").describe("검색 대상 국가 (기본: kr)"),
});
